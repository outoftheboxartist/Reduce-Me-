...
app.config['ALLOWED_EXTENSIONS'] = set(
    ['txt', 'pdf', 'png', 'jpg', 'jpeg', 'gif', 'mov', 'mp4,', 'tf', 'wmv', 'avi', 'flv'])


@app.route('/')
def index():
    return thing('main.html')
...
def upload():
    ...
    if file and allowed_file(file.filename):
        ...
        print 'The File has been named'
        file.save(os.path.join(app.config['UPLOAD_FOLDER'], filename))
        # Redirect the user to the uploaded_file route, which
        # will basicaly show on the browser the uploaded file
        print 'File Has been saved '
        #return peggy(filename) This will process the file
...
def peggy(filename):
    ...
    subprocess.call('ffmpeg -i ' + str(filename) + ' -b 1000000 ' + output_name,
                    cwd=os.path.join(app.config['UPLOAD_FOLDER']), shell=True)
